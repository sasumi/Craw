...
class Node {
	...
	/**
	 * check current node has any children
	 * @return bool
	 */
	...
	public function find_ancestor_tag($tag){
		...
		while(!is_null($ancestor)){
			Logger::instance(__CLASS__)->info('Current tag is: '.$ancestor->tag);
	...
	protected function seek($selector, &$ret, $parent_cmd, $lowercase = false){
		...
		// Go thought each element starting at this element until the end tag
		...
		foreach($nodes as $node){
			...
			if($pass && $attributes !== '' && is_array($attributes) && !empty($attributes)){
				foreach($attributes as $a){
					...
					Logger::instance(__CLASS__)->info('testing node: '.$node->tag.' for attribute: '.$att_name.$att_expr.$att_val.' where nodes value is: '.$nodeKeyValue);
					...
					Logger::instance(__CLASS__)->info('after match: '.($check ? 'true' : 'false'));
	...
	protected function parse_selector($selector_string){
		...
		Logger::instance(__CLASS__)->info('Matches Array: ', $matches);
	...
	public function addClass($class){
		...
		if(is_array($class)){
			...
			Logger::instance(__CLASS__)->info('Invalid type: ', gettype($class));
		}
	}
